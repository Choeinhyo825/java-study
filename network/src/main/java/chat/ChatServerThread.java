...
public class ChatServerThread extends Thread {
		...
		try {
			...
			while (true) {
				...
				if ("join".equalsIgnoreCase(tokens[0])) {
					...
					doQuit(printWriter);
					break;
				} else if ("to".equalsIgnoreCase(tokens[0]) && tokens.length > 2) {
					tomsg(tokens[1], tokens[2]);
				} else if ("ban".equalsIgnoreCase(tokens[0]) && tokens.length == 2) {
					ban(tokens[1]);
				} else {
					// ChatServer.log("에러:알수 없는 요청(" + tokens[0] + ")");
					if ("to".equalsIgnoreCase(tokens[0])) {
						printWriter.println("<<to 잘못된 명령어 입니다.>> ex)/to : 사용자 이름 : message/");
						printWriter.flush();
					} else if ("ban".equalsIgnoreCase(tokens[0])) {
						printWriter.println("<<dan 잘못된 명령어 입니다.>> ex)/dan : 사용자 이름/");
						printWriter.flush();
					}
				}
			}
	...
	private void suddenQuit(PrintWriter writer) {
		removeWriter(writer);
	}
		...
		if (listWriters.get(0) == this.printWriter) {
			...
			for (String key : map.keySet()) {
				if (!key.equalsIgnoreCase(name)) {
					...
					printWriter.println("<<" + name + "님을 추방되었습니다.>>");
				} else {
					PrintWriter printWriter = (PrintWriter) map.get(key);
			...
			PrintWriter printWriter = (PrintWriter) map.get(this.nickname);
			printWriter.println("<<당신은 방장이 아닙니다.>>");
			printWriter.flush();
		}
	}
	...
	private void doJoin(String nickName, Writer writer) {
		...
		if (listWriters.size() == 1) {
			String solo = "<<채팅방에 아무도 없습니다.>>";
			String king = "<<방장이 되었습니다.>>";
			broadcast(solo);
			broadcast(king);
		}
